...
double calcolo_norma(double vettore[], size_t dim);
unsigned int in_ordine_crescente(double vettore[], size_t dim);
...
int main() {
    ...
    double v[SIZE] = {0};
    unsigned int dimensione, input_utente, ordinato;
    double norma;
    ...
    do {
        ...
        switch (input_utente) {
            case 1:
                riempi_vettore(v, dimensione);
                break;

            case 2:
                norma = calcolo_norma(v, dimensione);
                printf("Norma: %.2f\n", norma);
                break;
            
            case 3:
                ordinato = in_ordine_crescente(v, dimensione);
